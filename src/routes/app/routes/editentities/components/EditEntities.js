...
class EditEntities extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            name: this.props.state.entity.entities_name,
            entities: this.props.state.entity.entities,
            group_id: this.props.state.entity.group_id
        };
    }
    updateEntities() {
        this.props.dispatch({
                        ...
                        <div className="card-action no-border text-center">
                            {
                                this.state.name && 
                                this.state.entities[this.state.entities.length - 1].length > 0 && 
                                <RaisedButton label="Сохранить" onClick={this.updateEntities.bind(this)} secondary />}
                        </div>
                    </div>
                </div>
            </div>
